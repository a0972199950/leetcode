...
**層級：判斷函式更複雜／二分結合其他結構**（1898 進行中，尚未算完成；1631、1482 待做）

`410`、`1011`、`875`、`1283` 這類典型「二分答案」題在 2026-05 做過，距今超過兩個月，不算本次複習已完成，但都在 8 個月內、不會被 `/q` 當新題選中。

- **1898 Maximum Number of Removable Characters 進行中，尚未算完成**：二分對象換成「移除幾個字元 k」、check function 換成「檢查子序列」都寫出來了，過了官方測資跟自己造的邊界測資，但二分邊界的 off-by-one（`while (right > left)` 漏掉 `left === right` 那個候選）跟後續的 `beingRemovedAt` 常數優化都是在 `/judge` 引導下才想到，使用者表示自己沒想到，不算站穩。下次 `/q` 或 `/judge` 1898 時，讓使用者自己重新推一次二分邊界（不要一開始就給 `left <= right` 的版本），才算這題完成。
...
← 目前在這裡：判斷函式更複雜 / 二分結合其他結構（1898 進行中，需重來一次二分邊界；1631、1482 待做）
```
