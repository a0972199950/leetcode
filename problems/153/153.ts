...
// Space: O(1)
// function findMin(nums: number[]): number {
//   // 每次二分，如果兩邊都有序遞增，那直接回傳 Math.min(min, nums[left])
//   // 如果一邊有序一邊無序，用有序那邊的最小更新 min 後，接著對無序的二分
...
//     const isLeftAscending = nums[middle] >= nums[left]
//     const isRightAscending = nums[right] >= nums[middle]

//     if (isLeftAscending && isRightAscending) {
...
function findMin(nums: number[]): number {
  ...
  // 不變式：最小值一定落在 [left, right] 內
  while (left < right) {
    const middle = Math.floor(left + (right - left) / 2)

    if (nums[middle] > nums[right]) {
      left = middle + 1 // 斷點在右半，最小值在 middle 右邊
    } else {
      right = middle // middle..right 有序，最小值是 middle 或更左
    }
  }
