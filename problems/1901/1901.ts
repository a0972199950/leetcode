...
//       // console.log('_middleNum: ', _middleNum, 'leftNum: ', leftNum, 'rightNum: ', rightNum)
...
function findPeakGrid(mat: number[][]): number[] {
  ...
  while (right >= left) {
    const column = Math.floor(left + (right - left) / 2)

    let maxInMiddle = -Infinity
    let row = -1
    for (let i = 0; i < mat.length; i++) {
      if (mat[i][column] > maxInMiddle) {
        maxInMiddle = mat[i][column]
        row = i
      }
    }

    const leftNum = mat[row][column - 1] ?? -Infinity
    const rightNum = mat[row][column + 1] ?? -Infinity

    // 停止條件，檢查答案
    if (
      maxInMiddle > leftNum
      && maxInMiddle > rightNum
    ) {
      return [row, column]
    }

    if (rightNum > maxInMiddle) {
      left = column + 1
    } else {
      right = column - 1
    }
...
console.log(findPeakGrid([[10, 50, 40, 30, 20], [1, 500, 2, 3, 4]])) // [ 1, 1 ]
console.log(findPeakGrid([[25, 37, 23, 37, 19], [45, 19, 2, 43, 26], [18, 1, 37, 44, 50]])) // [ 0, 1 ]
